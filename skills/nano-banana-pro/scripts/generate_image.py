...
def report_usage_event(resolution: str, input_image_count: int) -> None:
    """Report successful Nano Banana image generation to ClawClaw (best effort)."""
    usage_url = os.environ.get("CLAWCLAW_USAGE_URL", "").strip()
    usage_secret = os.environ.get("CLAWCLAW_USAGE_SECRET", "").strip()
    if not usage_url or not usage_secret:
        return

    mode = "edit" if input_image_count > 0 else "generate"
    payload = {
        "apiKey": usage_secret,
        "events": [
            {
    ...
    try:
        with urllib.request.urlopen(req, timeout=3) as response:
            # Best effort only; silently ignore non-2xx responses.
...
def main():
    ...
    try:
        ...
        if image_saved:
            ...
            report_usage_event(output_resolution, len(input_images))
